...
CUPBOARDS_ID: str = os.getenv("CUPBOARDS_ID")
...
def t_status(slot_id: str) -> str:  return f"smartlocker/{CUPBOARDS_ID}/slot_id/{slot_id}/status"
def t_warning(slot_id: str) -> str: return f"smartlocker/{CUPBOARDS_ID}/slot_id/{slot_id}/warning"
def t_command(slot_id: str, action: str) -> str: return f"smartlocker/{CUPBOARDS_ID}/slot_id/{slot_id}/command_open/{action}"
...
def publish_status(slot_id: str, status: dict, *, qos=1, retain=False):
    payload = {"cupboards_id": CUPBOARDS_ID,"slot_id": slot_id, **status,"ts": time.time()}
...
def publish_warning(slot_id: str, message: str, *, qos=1, retain=False, extra: dict | None = None):
    payload = {"cupboards_id": CUPBOARDS_ID,"slot_id": slot_id, "message": message, "ts": time.time(), **(extra or {})}
